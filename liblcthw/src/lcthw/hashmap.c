...
Hashmap *Hashmap_create(Hashmap_compare compare, Hashmap_hash hash, int buckets)
{
	...
	map->numbuckets = buckets > 0 ? buckets : DEFAULT_NUMBER_OF_BUCKETS;
	map->compare = compare == NULL ? default_compare : compare;
	map->hash = hash == NULL ? default_hash : hash;
	map->buckets = DArray_create(sizeof(DArray *), map->numbuckets);
		...
		int create, uint32_t *hash_out)
{
	...
	int bucket_n = hash % map->numbuckets;
...
{
	//int i = 0;
	//DArray_find(bucket, key, map->compare);
